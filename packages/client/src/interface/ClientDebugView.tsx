...
function isDebugUser(
  ...
  return username === "FTTristan" && discriminator === "0000";
}
...
export function ClientDebugView() {
  ...
  const allowed = createMemo(() =>
    isDebugUser(user()?.username, user()?.discriminator),
  );
  ...
  return (
    <Switch
      fallback={
        <Column style={{ padding: "20px", gap: "16px" }}>
          <h1 style={{ margin: 0, "font-size": "20px" }}>Client Debug View</h1>
          <Text>This view is restricted to FTTristan#0000.</Text>
        </Column>
      }
    >
      <Match when={allowed()}>
        <Column style={{ padding: "20px", gap: "12px", height: "100%" }}>
          <Row align style={{ "justify-content": "space-between", gap: "8px" }}>
            ...
            <Row align style={{ gap: "8px" }}>
              <Button
                variant="outlined"
                onPress={() => clearClientDebugEvents()}
              >
                Clear Logs
              </Button>
              <Button
